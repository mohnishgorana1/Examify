...
// here we will fetch all exams by a intructor and if exam has some submission then we will render those submission
function Results() {
  const { appUser } = useAppUser();
  const appUserId = appUser?._id;
  const [fetchedExamsWithSubmissions, setFetchedExamsWithSubmissions] =
    useState<any>();
  const [isFetchingExams, setIsFetchingExams] = useState(false);

  const fetchExamsWithSubmissions = async () => {
    if (!appUserId) return;
    setIsFetchingExams(true);
    try {
      const { data } = await axios.get(
        `/api/exam/instructor/exams/submissions?instructorId=${appUser?._id}`
      );
    ...
    <main className="w-full h-auto px-3 md:px-8 py-6 md:py-10">
      <section className="flex flex-col gap-y-6">
        ...
        <div className="flex flex-col gap-5">
          {fetchedExamsWithSubmissions &&
          fetchedExamsWithSubmissions.length === 0 ? (
            ...
            fetchedExamsWithSubmissions &&
            fetchedExamsWithSubmissions.length > 0 &&
            fetchedExamsWithSubmissions.map((exam: any) => (
              <div
                key={exam._id}
